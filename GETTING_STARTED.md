...
コマンドを実行するとコンテナが立ち上がり、起動直後は自動的にコンテナ内部のシェルに入ります。
...
* **--rm**: コンテナ終了時に「箱」を完全破棄。ホスト側の成果物は保持される
* **-it**: インタラクティブモード。**そのままコンテナ内の操作へ移るために必要**
* **--read-only**: ルートファイルシステム（/）を書き込み禁止にし、OS破壊を防止
* **--cap-drop ALL**: Linuxカーネルの権限を全削除し、権限昇格を防止
* **-v $(pwd):/app:rw**: ホストのソースコードを同期。成果物はホスト側にも保存される

> [!TIP]
> **コンテナ起動後に設定を変更する方法について**
> この環境は「毎回使い捨て」がコンセプトです。`exit` すると環境は消滅しますが、これにより常に不純物のない状態で実行できるメリットがあります。
> **設定変更のコツ**
> 起動したまま `.env` を修正したい場合は、**別のターミナルからWSLに入って編集**するのがおすすめです。マウント機能により、編集内容はコンテナ内へ即座に同期されます。

## 4. 疎通確認（Health Check）

> [!NOTE]
> 以下のテストは、必ず**コンテナ内部**で実行してください。
> WSL側にもフォルダが見えるため実行できてしまいますが、コンテナ用の依存関係をWSLで動かしても正しく疎通できているかの証明になりません。
...
準備ができたら、小人を呼び出しましょう。（※コンテナ内部のシェルで以下のコマンドを実行）
